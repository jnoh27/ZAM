...
export const BeatSequencer: React.FC<BeatSequencerProps> = ({ onBack }) => {
  ...
  const playersRef = useRef<Tone.Sampler | null>(null);
    ...
    const sequence = new Tone.Sequence((time, col) => {
      ...
      currentGrid.forEach((row, rowIndex) => {
        if (row[col]) {
          ...
          const samplerInstance = playersRef.current;
          if (samplerInstance) {
            if (soundType === 'kick') samplerInstance.triggerAttackRelease("C1", "8n", time);
            else if (soundType === 'snare') samplerInstance.triggerAttackRelease("D1", "8n", time);
            else if (soundType === 'hihat') samplerInstance.triggerAttackRelease("E1", "8n", time);
            else if (soundType === 'clap') samplerInstance.triggerAttackRelease("F1", "8n", time);
          }
        }
      });
  ...
  const toggleStep = (row: number, col: number) => {
    ...
    if (newGrid[row][col] && !isPlaying) {
      ...
      if (samplerInstance) {
        Tone.start().then(() => {
          if (soundType === 'kick') samplerInstance.triggerAttackRelease("C1", "8n");
          else if (soundType === 'snare') samplerInstance.triggerAttackRelease("D1", "8n");
          else if (soundType === 'hihat') samplerInstance.triggerAttackRelease("E1", "8n");
          else if (soundType === 'clap') samplerInstance.triggerAttackRelease("F1", "8n");
        });
      }
    }
  };
